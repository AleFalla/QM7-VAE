...
model_prop_ls=prop_ls_NN(latent_size=latent_size,prop_size=len(prop_list))
model_reco=base_VAE(input_dim=input_dim, latent_size=latent_size)

train_losses, val_losses, val_error, last_model_reco, last_model_prop_ls, test_error_1 = TRAIN_reco_prop_ls(model_reco,model_prop_ls,dataset_1, dataset_2, config_num=config_num, folder_name='VAE_prop_to_ls_NN/checkpoints_reco_prop_ls')
...
plt.ylabel('loss')
plt.tight_layout()
plt.savefig('./VAE_prop_to_ls_NN/plots/train_val_vs_epochs_reco_prop_ls_NN.pdf')
...
plt.tight_layout()
plt.savefig('./VAE_prop_to_ls_NN/plots/val_reco_error_prop_ls_NN.pdf')

PATH='./VAE_prop_to_ls_NN/reco_prop_ls_NN_trained_models/prop_ls_config_num_{}'.format(config_num)
...
file = open("VAE_prop_to_ls_NN/test_error_log.txt", "w")
