...
def TRAIN_reco(model, dataset, dataset_2, reset=True, batch_size=500, train_size=20000, learning_rate=1e-3, factor=.9, patience=2, config_num=1, folder_name='checkpoints'):
    ...
    while (optimizer.param_groups[0]['lr'])>1e-6:
        ...
        if e%50==0 and e!=0:
            ...
            train_losses.append(loss_tr_reco)
            val_losses.append(loss_tst)
...
def TRAIN_reco_prop_ls(model_reco, model_prop_ls, dataset, dataset_2, reset=True, batch_size=500, train_size=20000, learning_rate=1e-3, factor=.9, patience=2, config_num=1, folder_name='checkpoints'):
