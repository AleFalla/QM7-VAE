from Train_Test_utils import TEST_reco, TRAIN_reco_ls_prop
import pandas as pd
from Utils import pandas_to_dataset_mol_prop
from VAE import base_VAE
from separate_models import ls_prop_lin
import torch
...
prop_list=list(df.columns[df.columns != 'BoB'])[47:94] #comment this if you want all the properties
...
model_ls_prop=ls_prop_lin(latent_size=latent_size,prop_size=len(prop_list))
model_reco=base_VAE(input_dim=input_dim, latent_size=latent_size)

train_losses, val_losses, val_error, last_model_reco, last_model_prop_ls, test_error_1 = TRAIN_reco_ls_prop(model_reco,model_ls_prop,dataset_1, dataset_2, config_num=config_num, folder_name='VAE_ls_to_prop_lin/checkpoints_reco_ls_prop')
...
plt.ylabel('loss')
plt.tight_layout()
plt.savefig('./VAE_ls_to_prop_lin/plots/train_val_vs_epochs_reco_ls_prop_lin.pdf')
...
plt.tight_layout()
plt.savefig('./VAE_ls_to_prop_lin/plots/val_reco_error_ls_prop_lin.pdf')

PATH='./VAE_ls_to_prop_lin/reco_ls_prop_lin_trained_models/ls_prop_config_num_{}'.format(config_num)
torch.save(last_model_prop_ls.state_dict(), PATH)
model_prop_ls = ls_prop_lin(latent_size=latent_size,prop_size=len(prop_list))
model_prop_ls.load_state_dict(torch.load(PATH))

PATH='./VAE_ls_to_prop_lin/reco_ls_prop_lin_trained_models/reco_config_num_{}'.format(config_num)
...
file = open("VAE_ls_to_prop_lin/test_error_log.txt", "w")
